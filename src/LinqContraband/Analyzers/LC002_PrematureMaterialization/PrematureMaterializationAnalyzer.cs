...
namespace LinqContraband
{
    ...
    public class PrematureMaterializationAnalyzer : DiagnosticAnalyzer
    {
        ...
        private static readonly LocalizableString MessageFormat = "Calling '{0}' on materialized collection but source was IQueryable. This fetches all data before filtering.";
        private static readonly LocalizableString Description = "Ensure filtering happens before materialization (ToList, ToArray, etc).";
        ...
        private void AnalyzeInvocation(OperationAnalysisContext context)
        {
            ...
            IOperation? receiverOp = invocation.Instance ?? 
                                     (invocation.Arguments.Length > 0 ? invocation.Arguments[0].Value : null);

            while (receiverOp is IConversionOperation conversion)
            {
                receiverOp = conversion.Operand;
            }
            ...
            if (receiverOp is IInvocationOperation previousInvocation)
            {
                if (IsMaterializingMethod(previousInvocation.TargetMethod))
                {
                    ...
                    while (sourceOp is IConversionOperation conversion)
                    {
                    ...
                    {
                        context.ReportDiagnostic(Diagnostic.Create(Rule, invocation.Syntax.GetLocation(), methodSymbol.Name));
                    }
                }
            }
        }
