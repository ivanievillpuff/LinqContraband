...
namespace LinqContraband.Sample.Samples.LC004_IQueryableLeak
{
    ...
    /// <remarks>
    /// <para>
    /// <strong>The Crime:</strong> Passing an <c>IQueryable&lt;T&gt;</c> to a method that accepts <c>IEnumerable&lt;T&gt;</c>, 
    /// where that method then iterates the collection.
    /// </para>
    /// <para>
    /// <strong>Why it's bad:</strong> Accepting <c>IEnumerable&lt;T&gt;</c> signals "I am working with an in-memory collection".
    /// When you pass a database query to it, you lose the ability to add more SQL filters (like <c>Where</c> or <c>Take</c>) 
    /// <em>inside</em> the called method. The query is executed "as is" when iteration begins, potentially fetching too much data.
    /// </para>
    /// <para>
    /// <strong>The Fix:</strong> Change the parameter type to <c>IQueryable&lt;T&gt;</c> if the method intends to extend the query, 
